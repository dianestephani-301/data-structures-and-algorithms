...
const templateWithJQuery = () => {
  starWarsPeople.forEach(element => {
    let sectionTemplate = $('#template').html();
    let $newSection = $(`<section>${sectionTemplate}</section>`)
    $newSection.find('h2').text(element.name);
    $newSection.find('h3').text(element.height);
    $newSection.find('p').text(element.eye_color);
